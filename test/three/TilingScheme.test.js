...
describe( 'TilingScheme', () => {
	...
	it( 'should clamp bounds correctly.', () => {
		...
		expect( scheme.clampToNormalizedBounds( [ - 1, - 1, 2, 2 ], true ) ).toEqual( [ 0, 0, 1, 1 ] );
		expect( scheme.clampToNormalizedBounds( [ - 1, - 1, 5, 5 ], false ) ).toEqual( [ - 1, - 1, Math.PI, Math.PI / 2 ] );
		expect( scheme.clampToNormalizedBounds( [ - 5, - 5, 5, 5 ], false ) ).toEqual( [ - Math.PI, - Math.PI / 2, Math.PI, Math.PI / 2 ] );
