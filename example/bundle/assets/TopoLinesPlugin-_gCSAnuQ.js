import{V as l,C as d,k as w,l as m,i as p,g as j}from"./three.module-D7KO9fW1.js";const q=`
...
`;function $(r,t){const o={resolution:{value:new l},pixelRatio:{value:1},ellipsoid:{value:new m},frame:{value:new w},topoColor:{value:new d},topoOpacity:{value:.5},topoLimit:{value:new l(0,1e10)},topoFadeLimit:{value:new l(0,1e10)},cartoColor:{value:new d},cartoOpacity:{value:.5},cartoLimit:{value:new l(0,1e10)},cartoFadeLimit:{value:new l(0,1e10)},thickness:{value:1}};return r.defines={...r.defines||{},USE_TOPO_ELLIPSOID:0,USE_TOPO_LINES:0},r.onBeforeCompile=e=>{G(e),t&&t(e),e.uniforms={...e.uniforms,...o},e.vertexShader=e.vertexShader.replace(/void main\(/,i=>`
			...
			`)},o}function G(r){if(!/varying\s+vec3\s+wPosition/.test(r.vertexShader))return r.vertexShader=`
		...
		varying vec3 wPosition;
		${r.fragmentShader}
	`,r}const H=new m,V=new l,s=new m,S=new m,T=new m,W=new m,B=new m,J=new w,K=new w,h=new m,v=new m;class Q extends j{constructor(){super()}dispose(){this.removeFromParent(),this.material.dispose(),this.geometry.dispose()}}class Y{get resolution(){return this._resolution}get topoOpacity(){return this.topoOpacityUniform.value}set topoOpacity(t){this.topoOpacityUniform.value!==t&&(this.topoOpacityUniform.value=t,this.updateDefines())}get cartoOpacity(){return this.cartoOpacityUniform.value}set cartoOpacity(t){this.cartoOpacityUniform.value!==t&&(this.cartoOpacityUniform.value=t,this.updateDefines())}get projection(){return this._projection}set projection(t){t!==this._projection&&(this._projection=t,this.updateDefines())}get thickness(){return this.thicknessUniform.value}set thickness(t){this.thicknessUniform.value=t}constructor(t={}){const o="projection"in t?t.projection==="planar":!0,{projection:e="planar",thickness:i=1,topoColor:a=new d(16777215),topoOpacity:n=.5,topoLimit:c=o?new l(.1,1e10):new l(1,1e10),topoFadeLimit:L=o?new l(0,1e10):new l(0,4.25*1e3),cartoColor:P=new d(16777215),cartoOpacity:_=o?0:.5,cartoLimit:f=new l(.1,1e10),cartoFadeLimit:O=o?new l(0,1e10):new l(4.5*1e3,1e6)}=t;this.name="TOPO_LINES_PLUGIN",this.tiles=null,this.thicknessUniform={value:i},this.topoColor=new d().set(a),this.topoOpacityUniform={value:n},this.topoLimit=new l(...c),this.topoFadeLimit=new l(...L),this.cartoColor=new d().set(P),this.cartoOpacityUniform={value:_},this.cartoLimit=new l(...f),this.cartoFadeLimit=new l(...O),this._projection=e,this._pixelRatioUniform={value:1},this._resolution=new l(1,1),this._resolutionSampleObject=null}computeTopographicLineInfo(t,o,e={}){e.alpha=0,e.value=0,e.fade=0,e.emphasisStride=0,e.min=e.min||{step:0,stepInPixels:0},e.max=e.max||{step:0,stepInPixels:0};const{cartoLimit:i,cartoFadeLimit:a,topoLimit:n,topoFadeLimit:c}=this,L=this._pixelRatioUniform.value,P=this._resolution,_=this._projection,f=J.copy(this.tiles.group.matrixWorld).invert(),O=K.copy(t.matrixWorld).invert(),M=.25,x=M*.5,b=L*1.75,E=H.copy(o).applyMatrix4(O).z,C=W.set(0,0,E).applyMatrix4(t.projectionMatrix),D=B.set(1,1,E).applyMatrix4(t.projectionMatrix),F=V.subVectors(D,C).multiply(P).multiplyScalar(.5),g=Math.max(Math.abs(F.x),Math.abs(F.y)),U=1/g,z=b*U,y=Math.log10(z),N=1-p.smoothstep(1-Math.abs(y%1),1-M,1),u=Math.pow(10,Math.ceil(y));if(_==="ellipsoid"){s.copy(o).applyMatrix4(f);const I=this.tiles.ellipsoid;I.getPositionToSurfacePoint(s,T),I.getPositionToNormal(s,S);const R=s.distanceTo(T);s.x=Math.atan(S.y,S.x),s.y=Math.asin(S.z),s.z=R,s.x=s.x*p.RAD2DEG,s.y=s.x*p.RAD2DEG,s.x+=180,s.x*=1e3,s.y*=1e3}else s.copy(o).applyMatrix4(f),s.x*=.1,s.y*=.1;h.x=p.clamp(u,i.x,i.y),h.y=p.clamp(u,i.x,i.y),h.z=p.clamp(u,n.x,n.y),v.x=p.clamp(u*10,i.x,i.y),v.y=p.clamp(u*10,i.x,i.y),v.z=p.clamp(u*10,n.x,n.y);const k=1-p.smoothstep(c.y*(1-x),c.y*(1+x),Math.pow(10,y+1)),A=p.smoothstep(a.y*(1-x),a.y*(1+x),Math.pow(10,y+1));return e.alpha=N,e.fade=k*A,e.value=s.z,e.min.step=h.z,e.min.stepInPixels=h.z*g,e.max.step=v.z,e.max.stepInPixels=v.z*g,e}init(t){this.tiles=t;const o=new Q;o.frustumCulled=!1,o.onBeforeRender=e=>{const i=e.getRenderTarget();i?(this._resolution.width=i.width,this._resolution.height=i.height,this._pixelRatioUniform.value=1):(e.getDrawingBufferSize(this._resolution),this._pixelRatioUniform.value=e.getPixelRatio())},t.group.add(o),this._resolutionSampleObject=o}updateDefines(t=null){const o=+(this.projection==="ellipsoid"),e=+!!(this.topoOpacity+this.cartoOpacity),i=a=>{a.traverse(n=>{if(n.material){const{defines:c}=n.material;c.USE_TOPO_ELLIPSOID!==o&&(c.USE_TOPO_ELLIPSOID=o,n.material.needsUpdate=!0),c.USE_TOPO_LINES!==e&&(c.USE_TOPO_LINES=e,n.material.needsUpdate=!0)}})};t!==null?i(t):this.tiles.forEachLoadedModel(i)}processTileModel(t,o){const{tiles:e}=this;t.traverse(i=>{if(i.material){const a=$(i.material,i.material.onBeforeCompile);a.ellipsoid.value=e.ellipsoid.radius,a.frame.value=e.group.matrixWorld,a.thickness=this.thicknessUniform,a.topoColor.value=this.topoColor,a.topoOpacity=this.topoOpacityUniform,a.topoLimit.value=this.topoLimit,a.topoFadeLimit.value=this.topoFadeLimit,a.cartoColor.value=this.cartoColor,a.cartoOpacity=this.cartoOpacityUniform,a.cartoLimit.value=this.cartoLimit,a.cartoFadeLimit.value=this.cartoFadeLimit,a.resolution.value=this._resolution,a.pixelRatio=this._pixelRatioUniform,i.material.defines.USE_TOPO_ELLIPSOID=+(this.projection==="ellipsoid"),i.material.needsUpdate=!0}}),this.updateDefines(t)}dispose(){this.cartoOpacity=0,this.topoOpacity=0,this.updateDefines(),this._resolutionSampleObject.dispose()}}export{Y as T};
